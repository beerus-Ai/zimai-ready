...
const SERIF = '"EB Garamond", Georgia, "Times New Roman", "Noto Serif", serif';
const GOLD = { light: '#ffd6a3', mid: '#ffa946', deep: '#dc7110', dark: '#743912' };
const BRAND_DARK = '#07352f';
const BRAND = '#034f46';
const WINE = '#7f1c34';
...
export function CertificateCard({ certificate: c, className }: { certificate: Certificate; className?: string }) {
  ...
  return (
    <div
      className={cn('print-area relative w-full select-text overflow-hidden rounded-2xl bg-[#fffdf6] shadow-lift ring-1 ring-ink-950/10 print:rounded-none print:ring-0', className)}
      style={rootStyle}
      role="img"
      ...
      <div className="absolute inset-0 text-ink-950" style={{ fontSize: '1cqw' }}>
        ...
        <div className="absolute flex flex-col" style={{ top: '6.6em', left: '8em', right: '8em', bottom: '6.2em' }}>
          ...
          <div className="flex items-start justify-between gap-[2em]">
            <div className="flex items-center gap-[1.1em]">
              ...
              <div className="leading-none">
                <p className="text-[2.2em] font-extrabold tracking-tight">
                  ZimAI <span className="text-brand-800">Ready</span>
                </p>
            ...
            <div
              ...
              style={{ background: isEmployer ? WINE : BRAND, clipPath: 'polygon(0 0, 100% 0, 100% 100%, 0 100%, 1.4em 50%)', padding: '0.9em 1.6em 0.9em 2.8em' }}
            >
          ...
          <div className="mt-[2.4em] flex flex-col items-center text-center">
            <h2 className="flex items-center justify-center gap-[0.45em] whitespace-nowrap text-[2.7em] leading-none tracking-[0.01em] text-ink-950">
              <span
                className="inline-block font-condensed uppercase tracking-[0.05em]"
                style={{ color, border: '0.06em solid currentColor', borderRadius: '0.14em', padding: '0.1em 0.32em 0.06em' }}
              >
                {LEVEL_WORD[c.level]}
              </span>
              <span>
                Professional <em>Certification</em>
              </span>
            </h2>
            <AccentBar className="mx-auto mt-[1em] !h-[0.32em] !w-[18em]" />
            <p className="mt-[2.2em] text-[1.25em] font-semibold uppercase tracking-[0.3em] text-slate-500">This certifies that</p>
            <p className="mt-[0.35em] max-w-full truncate leading-[1.15] text-ink-950" style={{ fontFamily: SERIF, fontSize: nameSize, fontWeight: 500, letterSpacing: '-0.01em' }}>
              {c.holderName}
            </p>
          ...
          {/* Key facts */}
          <div className="mt-[2.4em] grid grid-cols-5 border-y border-slate-200" style={{ borderColor: tint('#1a1a1a', 0.1) }}>
          ...
          <div className="mt-auto grid items-end gap-[2.6em]" style={{ gridTemplateColumns: '1fr auto auto' }}>
            ...
            <div className="flex items-end gap-[1.2em]">
              <div className="text-right">
                ...
                <p className="mt-[0.2em] max-w-[17em] break-all text-[0.95em] font-semibold text-brand-800">{verifyDisplayUrl()}</p>
              </div>
              <div className="rounded-[0.6em] bg-white p-[0.55em] ring-1 ring-ink-950/10">
                <div className="h-[9.2em] w-[9.2em]">
                  <QRCodeSVG value={url} size={256} level="M" marginSize={0} fgColor="#1a1a1a" bgColor="#ffffff" style={{ width: '100%', height: '100%', display: 'block' }} title={`Verify certificate ${c.id}`} />
                </div>
              </div>
            </div>
          </div>
        ...
        {state !== 'valid' && (
          <div className="pointer-events-none absolute inset-0 flex items-center justify-center">
            <div
              className="rounded-[0.2em] bg-paper/70 px-[0.6em] pb-[0.05em] pt-[0.12em] font-condensed text-[6.4em] uppercase leading-none tracking-[0.12em]"
              style={{ color: state === 'revoked' ? '#d6361c' : '#dc7110', border: `0.08em solid currentColor`, transform: 'rotate(-14deg)', opacity: 0.85 }}
            >
              {state}
            </div>
          </div>
...
function Fact({ label, value, color, first }: { label: string; value: string; color?: string; first?: boolean }) {
  return (
    <div className={cn('px-[1.3em] py-[1.1em] text-center', !first && 'border-l')} style={{ borderColor: tint('#1a1a1a', 0.1) }}>
      <p className="text-[0.85em] font-bold uppercase tracking-[0.16em] text-slate-500">{label}</p>
      <p className="mt-[0.35em] truncate text-[1.5em] font-medium leading-tight text-ink-950" style={{ fontFamily: SERIF, ...(color ? { color } : {}) }}>
        {value}
      </p>
    </div>
  );
...
function Frame({ uid, color }: { uid: string; color: string }) {
  ...
  return (
    <svg viewBox="0 0 297 210" preserveAspectRatio="none" className="absolute inset-0 h-full w-full" aria-hidden>
      ...
      <rect width="297" height="210" fill="#fffdf6" />
...
function Seal({ uid, color, label, year }: { uid: string; color: string; label: string; year: number }) {
  ...
  return (
    <svg viewBox="0 0 120 142" className="h-[13.5em] w-[11.4em] shrink-0 drop-shadow-sm" aria-hidden>
      ...
      <polygon points={points} fill={`url(#${uid}-gold)`} stroke="#b6560f" strokeWidth="0.6" />
      ...
      <text x="60" y="71" textAnchor="middle" fontSize={label.length > 8 ? 8.2 : 9.5} fontWeight="800" fill="#3f1b06" letterSpacing="0.4">
        {label}
      </text>
