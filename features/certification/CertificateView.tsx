...
export default function CertificateView() {
  ...
  const back = (
    <Link to="/app/certificates" className="inline-flex items-center gap-1 hover:text-brand-700">
  ...
  return (
    <div>
      <PageHeader
        eyebrow={back}
        title={`${level.label} · ${cert.domainName}`}
      ...
      <div className="no-print mt-6 grid gap-4 lg:grid-cols-3">
        ...
        <div className="space-y-4">
          ...
          <Card>
            <CardTitle icon={<ShieldCheck className="h-5 w-5" />} title="Public verification" subtitle="Anyone can confirm this certificate." />
            <div className="flex items-center gap-4">
              <div className="shrink-0 rounded-xl bg-white p-2 ring-1 ring-slate-200">
                <QRCodeSVG value={url} size={96} marginSize={0} level="M" fgColor="#0b1220" />
              </div>
              <div className="min-w-0">
                ...
                <button onClick={copyLink} className="mt-2 inline-flex items-center gap-1 text-sm font-semibold text-brand-700 hover:underline">
...
function StatusBanner({ cert, state }: { cert: Certificate; state: CertDisplayState }) {
  ...
  const meta = {
    valid: { cls: 'border-brand-200 bg-brand-50/70 text-brand-900', icon: <ShieldCheck className="h-5 w-5 text-brand-600" />, text: 'Valid and publicly verifiable. Employers can scan the QR code or enter the certificate ID on the verification page.' },
  ...
  return (
    <div className={cn('no-print flex flex-col gap-3 rounded-2xl border px-4 py-3 sm:flex-row sm:items-center sm:justify-between', meta.cls)}>
  ...
  return (
    <div>
      <div className="flex items-baseline justify-between gap-3">
        ...
        <p className="text-sm font-bold tabular-nums text-ink-950">{score ? `${Math.round(score)}%` : '—'}</p>
      </div>
...
function EvidencePanel({ cert }: { cert: Certificate }) {
  ...
  return (
    <Card className="lg:col-span-2">
      <CardTitle icon={<ClipboardCheck className="h-5 w-5" />} title="Evidence behind this certificate" subtitle="Certification is awarded for demonstrated competency, not course completion." />
      ...
      <div className="mt-6 grid grid-cols-3 gap-3">
        {[
          ...
          <div key={s.label} className="rounded-xl bg-slate-50 p-3 sm:p-4">
            <p className="flex items-center gap-1.5 text-[11px] font-semibold uppercase tracking-wide text-slate-500 sm:text-xs">
              {s.icon}
              <span className="truncate">{s.label}</span>
            </p>
            <p className="mt-1 text-xl font-extrabold tabular-nums text-ink-950 sm:text-2xl">{s.value}</p>
          </div>
        ))}
      </div>
      <div className="mt-6">
...
function ExpiryCard({ cert, state }: { cert: Certificate; state: CertDisplayState }) {
  ...
  return (
    <Card>
      <CardTitle icon={<CalendarClock className="h-5 w-5" />} title="Validity" subtitle="Certificates are valid for 12 months." />
      <div className="flex items-baseline gap-2">
        <span className={cn('text-4xl font-extrabold tabular-nums tracking-tight', inactive ? 'text-slate-400' : state === 'expiring' ? 'text-gold-600' : 'text-ink-950')}>
      ...
      <p className="mt-4 text-sm text-slate-600">AI readiness is not permanent. Keep your skills current and renew before your certificate expires.</p>
