...
import { HeroArt, SampleDataBanner } from './components';
...
export default function CompetenciesPage() {
  ...
  return (
    <div className="animate-fade-in">
      <PageHeader
        eyebrow={org.name}
        title={<>Required AI <em>competencies</em></>}
        description={`Define what “AI ready” means for ${org.name}.`}
        actions={
          <div className="flex items-end gap-5">
            <HeroArt>
              <MapPins className="h-28 w-28 lg:h-32 lg:w-32" animated />
            </HeroArt>
      ...
      <Reveal>
      <Card className="mb-8 rounded-3xl border-0 bg-sand-100 shadow-none sm:p-8">
    ...
    <div className="grid gap-5 md:grid-cols-2">
      {sorted.map((c, i) => {
        ...
        return (
          ...
          <Card className="flex h-full flex-col rounded-3xl transition duration-300 hover:-translate-y-0.5 hover:border-ink-950/30 hover:shadow-ink-sm sm:p-7">
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="flex flex-wrap items-center gap-2">
                  <h3 className="font-display text-xl font-medium leading-tight text-ink-950">{c.name}</h3>
              ...
              <div className="flex shrink-0 gap-1">
                <button type="button" onClick={() => onEdit(c)} className="rounded-lg p-2 text-slate-400 hover:bg-sand-200/60 hover:text-ink-900" aria-label={`Edit ${c.name}`}>
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              {c.skillIds.map((s) => (
                <span key={s} className="rounded-full border border-ink-950/10 bg-sand-100 px-2.5 py-0.5 text-xs font-medium text-ink-700">
                  {skillName(s)}
                </span>
              ))}
            </div>
              ...
              <div className="mb-1.5 flex items-center justify-between text-xs">
                ...
                {cov && (
                  <span className="font-bold tabular-nums text-ink-950">
                    <span className="font-display text-lg font-medium">{cov.pct}%</span> meet it <span className="font-medium text-slate-400">({cov.met}/{members.length})</span>
                  </span>
                )}
              </div>
              {cov && (
                <div className="h-2 overflow-hidden rounded-full bg-sand-200">
                  <div className="h-full rounded-full transition-[width] duration-700" style={{ width: `${cov.pct}%`, background: cov.pct >= 60 ? '#034f46' : cov.pct >= 40 ? '#ffa946' : '#ff6c4c' }} />
                </div>
              )}
            </div>
          </Card>
...
function DepartmentComparison({ org, members }: { org: Organisation; members: WorkforceMember[] }) {
  ...
  return (
    ...
    <Card className="sm:p-8">
      <CardTitle title="Workforce vs your framework" subtitle="% meeting each competency, overall and by department" />
      <Heatmap
      ...
      <div className="space-y-5">
        <label className="block">
          ...
          <input
            ...
            maxLength={70}
            className={cn('h-11 w-full rounded-xl border bg-paper px-3 text-sm focus:outline-none focus:ring-2', show('name') ? 'border-clay-400 focus:ring-clay-200' : 'border-ink-950/15 focus:border-ink-950 focus:ring-lilac-200')}
        ...
        <label className="block">
          ...
          <textarea
            ...
            className={cn('w-full rounded-xl border bg-paper px-3 py-2.5 text-sm focus:outline-none focus:ring-2', show('description') ? 'border-clay-400 focus:ring-clay-200' : 'border-ink-950/15 focus:border-ink-950 focus:ring-lilac-200')}
        ...
        <div>
          ...
          <div className="flex max-h-56 flex-wrap gap-2 overflow-y-auto rounded-xl border border-ink-950/10 bg-sand-100 p-3">
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <fieldset>
            ...
            <div className="grid grid-cols-3 gap-1 rounded-xl border border-ink-950/10 bg-sand-200/60 p-1">
              {([1, 2, 3] as SkillLevel[]).map((l) => (
                <button
                  ...
                  className={cn('rounded-lg px-2 py-2 text-xs font-semibold transition', d.requiredLevel === l ? 'border border-ink-950 bg-lilac-200 text-ink-950 shadow-ink-sm' : 'text-slate-500 hover:text-ink-900')}
                >
            ...
            <div className="grid grid-cols-3 gap-1 rounded-xl border border-ink-950/10 bg-sand-200/60 p-1">
              {PRIORITIES.map((p) => (
                <button
                  ...
                  className={cn('rounded-lg px-2 py-2 text-xs font-semibold transition', d.priority === p ? 'border border-ink-950 bg-lilac-200 text-ink-950 shadow-ink-sm' : 'text-slate-500 hover:text-ink-900')}
                >
  ...
  return (
    ...
    >
      <label className="block">
        ...
        <select value={choice} onChange={(e) => setChoice(e.target.value)} className="h-11 w-full rounded-xl border border-ink-950/15 bg-paper px-3 text-sm focus:border-ink-950 focus:outline-none focus:ring-2 focus:ring-lilac-200">
      ...
      <ul className="mt-4 space-y-2">
        {tpl.competencies.map(([name, description, skillIds, level, priority]) => (
          <li key={name} className="rounded-2xl border border-ink-950/10 bg-sand-100 p-3">
            <div className="flex flex-wrap items-center gap-2">
